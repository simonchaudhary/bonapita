import {Container} from "./PreplineManagerStyle";
import {
	Accordion,
	AccordionItem,
...
function PrepLineManager() {
	...
	useEffect(() => {
		...
		// const date4 = new Date("1/7/2021, 7:2:42 AM");
	...
	const get = async () => {
		axios
			.get(
				"https://us-central1-afoodie-6d649.cloudfunctions.net/order/orders-items/prepline-manager?providerId=" +
					user?.provider
			)
			.then((res) => {
				...
				orderList.map((orderItem, i) => {
					...
					// modifyDate(orderItem.date);
					// modifyTime(orderItem.time);
...
export default withRouter(PrepLineManager);
