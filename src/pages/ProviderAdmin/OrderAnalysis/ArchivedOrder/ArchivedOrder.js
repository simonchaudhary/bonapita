...
function ArchivedOrder() {
	...
	const getSalesByFilter = () => {
		...
		if (filterDay !== "null") {
			...
			axios
				.get(
					"https://us-central1-afoodie-6d649.cloudfunctions.net/orderAnalysis-archive/" +
						filterDay +
						"?providerId=" +
						user?.provider
				)
	...
	const getSalesByRange = (start, end) => {
		...
		axios
			.get(
				"https://us-central1-afoodie-6d649.cloudfunctions.net/orderAnalysis-archive/range?start=" +
					start +
					"&end=" +
					end +
					"&providerId=" +
